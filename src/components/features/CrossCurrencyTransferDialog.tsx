...
export function CrossCurrencyTransferDialog({ open, onOpenChange, onSuccess }: { open: boolean, onOpenChange: (open: boolean) => void, onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !sourceAccountId || !destAccountId || !grossAmount || !inrAmount) return;
    
    const parsedUSD = parseFloat(grossAmount);
    const parsedINR = parseFloat(inrAmount);
    const effectiveExchangeRate = parsedUSD > 0 ? parsedINR / parsedUSD : 0;
      ...
      <DialogContent className="sm:max-w-[480px] w-[95vw] bg-card/95 backdrop-blur-2xl border-indigo-500/20 shadow-2xl rounded-3xl overflow-y-auto max-h-[90dvh] p-0 gap-0">
        ...
        <div className="p-6 pb-4">
          ...
          <form id="transfer-form" onSubmit={handleSubmit} className="space-y-4 relative z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="amount" className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/90">Amount Withdrawn (USD)</Label>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="inrAmount" className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/90">Amount Received (INR)</Label>
                <div className="relative flex items-center">
                  <div className="absolute left-3 text-muted-foreground">
                  ...
                  <Input 
                    ...
                    placeholder="0.00"
                    className="text-left text-2xl font-bold h-14 rounded-xl bg-background/50 border-border/60 focus-visible:ring-indigo-500/30 pl-10 text-emerald-500"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="p-6 pt-4 bg-muted/30 border-t border-border/40 mt-2">
          <Button 
            ...
            disabled={loading || !sourceAccountId || !destAccountId || !grossAmount || !inrAmount}
          >
